 ...
 * to determine a path, that also avoids collision with a mover whose path is known. 
 * 
 */
public class AStarNoCollision implements PathFinder {
	...
	/** The path of the second mover that we're avoiding collisions with */
	private Path robot;
	...
	public AStarPathFinder(TileBasedMap map, int maxSearchDistance, 
						   boolean allowDiagMovement, AStarHeuristic heuristic, Path path) {
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			for (int x=-1;x<2;x++) {
				for (int y=-1;y<2;y++) {
					...
					if ((isValidLocation(mover,sx,sy,xp,yp)) && ((xp != robot.getX(current.getDepth)) || (yp != robot.getY(current.getDepth))) { //Second condition checks for collisions
